...
# ═══════════════ SIDEBAR: PILIH SEKTOR ═══════════════
with st.sidebar:
    st.markdown("## 🎯 Pilih Sektor")
    st.caption("Fokus pindai 1 sektor secara mendalam")
    sektor_list = list(SEKTOR_LENGKAP.keys())
    pilihan = st.selectbox("Sektor:", sektor_list,
    ...
    st.success(f"📊 **{n_saham} saham** di sektor ini")

    # Tampilkan daftar saham di sektor itu
    with st.expander("📋 Lihat daftar saham sektor ini"):
        saham_list = [s.replace(".JK","") for s in SEKTOR_LENGKAP[pilihan]]
        st.markdown(" · ".join([f"**{s}**" for s in saham_list]))

    st.markdown("---")
    st.markdown("### 📚 Semua Sektor")
    st.caption("Klik untuk pindah:")
    for sek in sektor_list:
        n = len(SEKTOR_LENGKAP[sek])
        is_active = sek == st.session_state.sektor_pilih
        prefix = "✅ " if is_active else "▫️ "
        if st.button(f"{prefix}{sek} ({n})", use_container_width=True, key=f"btn_{sek}",
                     disabled=is_active):
            st.session_state.sektor_pilih = sek
            st.session_state.results = None
            st.cache_data.clear()
            st.rerun()

    st.markdown("---")
    st.caption("💡 **Tips:** Fokus 1 sektor lebih dalam = pemahaman lebih baik. Bandingkan saham dalam sektor yang sama (apple to apple).")

# ═══════════════ MAIN APP ═══════════════
c1,c2 = st.columns([3,1])
with c1:
    st.markdown(f"# {st.session_state.sektor_pilih}")
    st.caption(f"Scanner Sektor · Swing 1-6 hari · Target 5%+ · {datetime.now().strftime('%d %b %Y · %H:%M')}")
    st.caption(f"📊 Memindai **{len(WATCHLIST)} saham** di sektor ini")
with c2:
